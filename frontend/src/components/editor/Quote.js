...
export default function Quote(props) {
  ...
  const scanText = async image => {
    ...
    const result = await Tesseract.recognize(image, 'eng').progress(p => {});
    Tesseract.terminate();
      ...
      >
        {isVisible ? (
          <div className="quote-container">
              ...
              <div className="quote-editor">
                <div>
                  <div className="quote-menu">
                    ...
                    <button onClick={() => setIsFormatVisible(!isFormatVisible)}>
                  ...
                  <div className="add-to-reference">
                    <select name="refrence" id="reference">
                      {references.map(({ _id, authors, title }) => (
                        <option key={_id} value={_id}>{`${authors[0].lastName}, ${authors[0].firstName.charAt(
                          0
                        )}. ${title}`}</option>
                      ))}
                    </select>
            ...
            {isCameraActive ? (
              <Camera className="camera-quote" onTakePhoto={handlePhoto} isFullscreen={true} isImageMirror={false} />
            ) : (
              undefined
            )}
          </div>
